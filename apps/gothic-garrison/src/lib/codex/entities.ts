...
export const CODEX_ENTITIES: CodexEntity[] = [
  ...
  },
  {
    ...
    label: 'Attributes',
    singular: 'attribute',
    columns: ['name', 'pickScope', 'costDelta', 'rules'],
    fields: [
      { key: 'name', label: 'Name', type: 'text', required: true, sourceBadge: true },
      { key: 'sourceId', label: 'Source', type: 'source', required: true },
      { key: 'pickScope', label: 'Pick scope', type: 'enum', options: ['none', 'officer', 'soldier', 'any'], required: true, widget: 'tag-group', help: 'none=fixed on soldier type; officer=free pick pool; soldier=any soldier buys it; any=both.' },
      { key: 'costDelta', label: 'Cost (+pts)', type: 'number', min: 0, max: 99, help: 'Extra recruitment points when a soldier purchases this attribute.' },
